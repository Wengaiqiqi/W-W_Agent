#!/usr/bin/env python3
"""启动 W&W Agent 飞书 Gateway (使用系统 Python)"""

import paramiko
import os
import sys
import io
import time

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8')

def main():
    hostname = os.getenv("WW_HOST", "47.86.26.185")
    username = os.getenv("WW_USER", "root")
    password = os.getenv("WW_PASS")
    if not password:
        raise SystemExit("错误: 请设置环境变量 WW_PASS 提供服务器密码")

    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    try:
        print(f"连接到 {hostname}...")
        client.connect(hostname, 22, username, password, timeout=15, banner_timeout=30)
        print("[OK] SSH 连接成功")

        # 1. 检查 Python 环境
        print("\n--- 检查 Python ---")
        stdin, stdout, stderr = client.exec_command(
            "which python3 && python3 --version",
            timeout=10
        )
        print(stdout.read().decode('utf-8', errors='ignore').strip())

        # 2. 检查依赖
        print("\n--- 检查依赖 ---")
        stdin, stdout, stderr = client.exec_command(
            "cd '/root/W&W Agent' && python3 -c 'import httpx; print(\"httpx ok\")' 2>&1",
            timeout=10
        )
        print(stdout.read().decode('utf-8', errors='ignore').strip())

        # 3. 停止旧进程
        print("\n--- 停止旧进程 ---")
        stdin, stdout, stderr = client.exec_command(
            "pkill -f 'gateway.*feishu' 2>/dev/null; pkill -f 'python.*gateway' 2>/dev/null; echo '已清理'",
            timeout=10
        )
        print(stdout.read().decode('utf-8', errors='ignore').strip())

        # 4. 启动飞书 Gateway (并发数 4)
        print("\n--- 启动飞书 Gateway (并发数 4) ---")
        cmd = """cd '/root/W&W Agent' && nohup python3 -m gateway feishu --concurrency 4 > /root/feishu_gateway.log 2>&1 &"""
        stdin, stdout, stderr = client.exec_command(cmd, timeout=10)
        print("启动命令已执行")

        # 5. 等待启动
        print("\n--- 等待启动 ---")
        time.sleep(5)

        # 6. 检查进程
        print("\n--- 检查进程 ---")
        stdin, stdout, stderr = client.exec_command(
            "ps aux | grep 'gateway.*feishu' | grep -v grep",
            timeout=10
        )
        output = stdout.read().decode('utf-8', errors='ignore').strip()
        print(output if output else "无相关进程")

        # 7. 检查日志
        print("\n--- Gateway 日志 ---")
        stdin, stdout, stderr = client.exec_command(
            "tail -30 /root/feishu_gateway.log 2>/dev/null",
            timeout=10
        )
        print(stdout.read().decode('utf-8', errors='ignore').strip())

    except Exception as e:
        print(f"[ERROR] {type(e).__name__}: {e}")
    finally:
        try:
            client.close()
        except:
            pass

if __name__ == "__main__":
    main()
